...
const sendResetPasswordEmail = async (to: string, newPassword: string) => {
  ...
  await sendEmail(to, 'issoufoudjib@gmail.com', subject, text, html);
};

const sendVerificationEmail = async (to: string, token: string) => {
  const subject = 'Email Verification';
  const verificationEmailUrl = `${config.verificationEmailUrl}:${config.port}/rest/auth/verify-email/${token}`;
  const text = `Dear user,
