...
const GlucoseChart = (props: Props) => {
    const [tooltip,setTooltip] = useState({tooltipX: null,
        tooltipY: null,
        tooltipIndex: null,
      }) 
    ...
    <View>
      {GlucoseLevelData.data.filter((_,i)=>(i%2)===1).map((item, index) => (
        <Circle
          key={index}
  ...
  return (
    <View style={{backgroundColor: '#fff', marginTop: 20, paddingTop: 15}}>
        <View style={{ height: 220, paddingHorizontal: 10, flexDirection: "row" }}>
        <YAxis
            style={{ marginBottom: xAxisHeight}}
            data={GlucoseLevelData.data}
        ...
        <View style={{ flex: 1, marginLeft: 10 }}>
            ...
            > 
                ...
                <Tooltip
                          ...
                          dataLength={GlucoseLevelData.data.length} />
            </LineChart>
            <XAxis
                data={GlucoseLevelData.data.filter((_,i)=>(i%3)===1)}
                svg={{
                    fill: colors.Text,
                    fontSize: 12,
                    fontWeight: "bold",
