...
const GlucoseLevel = (props: Props) => {
  ...
  return (
    <ScrollView contentContainerStyle={[styles.Container]} scrollEnabled>
        ...
        <View style={styles.MarketPlacePlace}>
          <FrequencySelectionBar inOrder={true} selected={frequency} setSelected={setFrequency} bgColor={'#FCE8E9'} />
          <Text style={ExtraStyling.title}>Glucose Level</Text>
          <Text style={[ExtraStyling.title,ExtraStyling.date]}>{moment(new Date()).format('dddd, MMMM d, yyyy')}</Text>
          <GlucoseChart />
        </View>
    </ScrollView>
  )
